...
public:
    ...
    std::vector<Triangle> edgesToTriangles(const std::vector<Edge> &dt, std::vector<Triangle> &triangles);

    static void normalizePoints(std::vector<QPoint3DF>& points, int width, int height);
    static QPointF calculateCentroid(const std::vector<QPoint3DF>& points);
};
